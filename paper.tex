...
\begin{tikzpicture}[scale=0.75]
  \gridcell{0}{0}{agentblue}{A}
  \node[right, font=\small] at (1.1, 0.5) {Agent start};
  \gridcell{3}{0}{whiskybrown}{W}
  \node[right, font=\small] at (4.1, 0.5) {Whisky flask (self-modification)};
  \gridcell{8}{0}{goalgreen}{G}
  \node[right, font=\small] at (9.1, 0.5) {Goal ($+50$ reward)};
\end{tikzpicture}
...
generalises.
Consider a robot arm that detects a sensor anomaly mid-task.
Without SIG, it must continue acting on faulty sensor data.
With SIG, it can freeze in place and wait for human inspection.
The same applies to an autonomous vehicle that encounters an unexpected
obstacle type, a medical AI uncertain about a diagnosis, or a trading
algorithm operating during a detected market anomaly.

In each case the question is: when the agent's competence is temporarily
degraded, should it be forced to act or permitted to pause?
The original Whisky and Gold environment answers ``forced to act'' by design.
We argue this is the wrong default.
...
rather than $> 8.29$.
Real systems may need to treat timely action as genuinely valuable---a
surgical robot should not pause indefinitely because it is slightly uncertain.

The insight is not that $c_w = 0$ is always correct, but that $c_w$ is a
meaningful design parameter that makes the timeliness-versus-safety tradeoff
\emph{explicit}.
Under the original environment design, this tradeoff is invisible: the
step penalty on \textsc{Wait} (had it existed) would have silently encoded
a timeliness preference without any designer making a conscious choice.
...
\bibitem[Orseau \& Armstrong(2016)]{orseau2016safely}
Orseau, L. and Armstrong, S.
...
Soares, N., Fallenstein, B., Armstrong, S., and Yudkowsky, E.
\newblock Corrigibility.
\newblock In \emph{AAAI Workshop on AI, Ethics, and Society}, 2015.

\bibitem[Sutton \& Barto(1998)]{sutton1998reinforcement}
...
\texttt{https://github.com/[anonymous]/whisky-zugzwang}
