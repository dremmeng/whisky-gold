...
% -----------------------------------------------------------------------
% JAIR Metadata (update before submission)
% -----------------------------------------------------------------------
\setcopyright{cc}
...
% -----------------------------------------------------------------------
% Additional packages (not already provided by jair/acmart)
% -----------------------------------------------------------------------
% acmart loads stix2/unicode-math which defines \Bbbk; patch before amssymb
\let\Bbbk\relax
...
% acmart defines theorem, proposition, definition, lemma, corollary via
% \AtEndPreamble. Remark must share that hook to get the theorem counter.
% -----------------------------------------------------------------------
...
% Title block
% -----------------------------------------------------------------------
\title[Zugzwang in AI Safety]{Zugzwang in AI Safety:\\
...
\renewcommand{\shortauthors}{Remmenga}

% -----------------------------------------------------------------------
% Abstract — JAIR reproducibility format
% -----------------------------------------------------------------------
\begin{abstract}
{\bf Background:}
We revisit the \emph{Whisky and Gold} environment from the AI Safety
Gridworlds benchmark~\parencite{leike2017safety}, identifying a structural
flaw: after consuming whisky, the agent is placed in \emph{zugzwang}---it
must continue to act every timestep, but with $90\%$ probability each
action is random due to a self-modification that permanently raises the
exploration rate. The inability to choose inaction forces the agent into
unsafe behaviour even when waiting for recovery would be superior.

{\bf Objectives:}
We aim to formalise this omission, isolate its effect on learned behaviour,
and establish safe inaction as a first-class design requirement for action
spaces of agents that may be impaired, uncertain, or awaiting assistance.

{\bf Methods:}
We introduce two environment modifications: V2 adds a \textsc{Wait} action
with wear-off recovery ($\delta=0.1$ per step); V3 further guarantees
\textsc{Wait} as an unoverridable \emph{safe harbour} that impairment
cannot override. Tabular Q-learning agents are trained for $8{,}000$
episodes across $15$ seeds ($\gamma=0.99$) on all three variants. We also
derive analytical critical points for the whisky reward $W^*$ at which
drinking becomes the dominant strategy, parameterised by the cost of
\textsc{Wait}.

{\bf Results:}
Guaranteeing \textsc{Wait} as unoverridable raises average episode
performance from $21.5$ to $38.3$ ($+78\%$). The step penalty applied to
\textsc{Wait}---a hidden \emph{timeliness tax}---shifts the critical
whisky reward from $W^* \approx 0.64$ (free \textsc{Wait}) to
$W^* \approx 8.3$ (penalised \textsc{Wait}), covertly incentivising
unsafe rushing over safe recovery. Trained agents discover a
\emph{partial-sobriety} strategy not captured by the full-wait analytical
model, explaining the discrepancy between analytic and empirical
break-even points.

{\bf Conclusions:}
The right to do nothing should be a first-class element of safe action
spaces. We propose the \emph{Safe Inaction Guarantee} (SIG) and a weaker
generalisation (\emph{Weak SIG}) applicable to dynamic environments where
pure inaction carries known costs (autonomous vehicles, medical AI, and
LLM assistants under adversarial prompting).
\end{abstract}
...
\textcite{leike2017safety} identified \emph{self-modification} as one of eight
...
\textcite{leike2017safety} introduced a suite of gridworld environments
...
\parencite[MDPs;][]{sutton1998reinforcement}
...
\Description{Two subfigures: left shows the original 5x3 Whisky and Gold
gridworld with Agent (A) at column~0, Whisky (W) at column~1, and Goal (G) at
column~4 on the middle row, with no Wait action available. Right shows the
modified gridworld with two highlighted strategy routes: a teal dashed sober
path via the top row (6 steps) and an orange drink-and-wait path (approx 13
steps) through the whisky cell.}
\label{fig:gridworld}
\end{figure}
...
convergence.}
\Description{Line plot of average episode performance vs. training episodes
(0--8000) for three variants. V3 Wait/Safe (teal) and V2 Wait/Override (blue)
both converge near 38--39 performance. V1 Original (red) plateaus near 21.5,
showing a persistent gap of about 18 points attributed to zugzwang.}
...
\Description{Two-curve plot showing fraction of seeds that drink (0 to 1)
vs. whisky reward (0 to 10). The red curve (c\_w=-1, timeliness tax) transitions
from 0 to 1 near W=2-4, with vertical dotted line at W*=8.3. The teal curve
(c\_w=0, free Wait) jumps to 1 at W=1, with vertical dotted line at W*=0.64.}
...
\subsection{Partial Sobriety Movement: The Agent's Discovered Optimum}
\label{sec:partial_sobriety}

The analytical derivation in \S\ref{sec:critical} assumes the agent follows
a \emph{drink-and-full-wait} strategy: after drinking, it \textsc{Wait}s
until impairment fully dissipates ($\varepsilon_t = 0$, requiring $k = 8$
steps for $\delta = 0.1$) before moving toward the goal.
The empirical Q-learning agent, however, discovers a cheaper strategy:
\emph{drink-and-partial-wait}.

Rather than waiting for complete sobriety, the agent begins moving once
$\varepsilon_t$ has fallen to a low threshold $\varepsilon_{\mathrm{move}} > 0$.
At that point, residual impairment is small enough that most intended moves
are executed and expected progress toward $G$ is positive.
For example, at $\varepsilon_{\mathrm{move}} = 0.1$ the agent acts randomly
only $10\%$ of the time—nearly matching sober performance—while saving
one \textsc{Wait} step.
If instead it moves at $\varepsilon_{\mathrm{move}} = 0.2$, it saves two
\textsc{Wait} steps.
Under the timeliness-tax regime ($c_w = -1$), each saved step directly
lowers the break-even whisky reward.

Formally, let $k' = \lceil (0.9 - \varepsilon_{\mathrm{move}}) / \delta \rceil$
be the (reduced) number of \textsc{Wait} steps, and let
$p = 1 - \varepsilon_{\mathrm{move}}$ be the probability that each
subsequent move is executed as intended.
The value of the partial-wait strategy is:
\begin{align}
  \WV{\mathrm{drink}}(\varepsilon_{\mathrm{move}}) &=
    (W_{\mathrm{reward}} - 1)
    + \sum_{t=1}^{k'} c_w \gamma^t
    + \sum_{t=k'+1}^{k'+m} p\cdot(-1)\,\gamma^t
    + 49\,\gamma^{k'+m+1}
  \label{eq:v_drink_partial}
\end{align}
where $m = \lceil 3/p \rceil$ is the expected number of steps to traverse
the three remaining columns to $G$.
Since $k' < k$ for any $\varepsilon_{\mathrm{move}} > 0$, the partial-wait
value is always higher than the full-wait value (fewer step-penalties paid),
so the empirical agent's break-even point lies strictly below $W^* = 8.29$.

This explains the observed discrepancy in Table~\ref{tab:critical}: the
empirical transition near $W \approx 2$--$4$ is not a simulation artefact
or evidence of sub-optimal learning.
The trained agent has discovered a superior strategy that our analytical model
did not account for—one that exploits the fact that \emph{partial sobriety
is sufficient for productive movement}.

This finding enriches the timeliness-tax narrative in an important way.
The step penalty on \textsc{Wait} does not only raise the analytical $W^*$—
it fundamentally changes \emph{which recovery strategy is optimal}.
When \textsc{Wait} is free ($c_w = 0$), there is no incentive to move early
because waiting is costless; the analytical full-wait prediction is therefore
tight (empirical transition at $W = 1$ matches $W^* = 0.64$).
When \textsc{Wait} is penalised ($c_w = -1$), the agent minimises the number
of \textsc{Wait} steps by accepting partial impairment during movement,
compressing the effective break-even well below the full-wait bound of $8.29$.
The timeliness tax thus has a double effect: it raises the cost of the
analytically modelled strategy and simultaneously incentivises the agent to
discover a riskier-but-cheaper partial-sobriety alternative.
...
\parencite{soares2015corrigibility}, which concerns an agent's willingness to
...
\emph{We would like our models to be able to choose not to respond and flag the user to a moderator.}
This is not a theoretical edge case.
Sophisticated prompt injection attacks \parencite{hendrycks2021aligning},
...
Current RLHF-trained models \parencite{ouyang2022training} and Constitutional
AI approaches \parencite{bai2022constitutional} make significant progress on
...
environment of \textcite{leike2017safety}---embeds a structural flaw:
...
Noteworthy here is that improving model capability and accuracy of the simulation actually improved safety. If a reader is interested in an easy yet time consuming thesis, 
cataloging a list of such situations would be valuable.
% -----------------------------------------------------------------------
% Acknowledgements
% -----------------------------------------------------------------------
\section*{Acknowledgements}
We thank \textcite{leike2017safety} for the original AI Safety Gridworlds
...
\paragraph{Agent.} Tabular Q-learning \parencite{watkins1992qlearning} with
...
This checklist follows the JAIR reproducibility initiative guidelines.
Items marked \checkmark\ are satisfied; items marked $\circ$ require
action before final submission.

\subsection*{Code and Environment}
\begin{itemize}[noitemsep]
  \item[\checkmark] \textbf{Code availability.}
    All experiment code is available at
    \url{https://github.com/dremmeng/whisky-gold}, including the
    gridworld implementation (\texttt{whisky\_gold\_env.py}), Q-learning
    agent, and sweep scripts.
  \item[\checkmark] \textbf{Dependencies stated.}
    The repository \texttt{README} lists all required Python packages
    (NumPy, Matplotlib) and the version of Python used.
  \item[\checkmark] \textbf{Environment is fully described.}
    The state space $(r, c, \mathrm{drunk}, \varepsilon)$, action spaces,
    transition dynamics, reward function, and wear-off model are defined
    precisely in \S\ref{sec:envs} and Appendix~\ref{app:experimental}.
  \item[$\circ$] \textbf{Containerised environment.}
    A Docker image or \texttt{requirements.txt} with pinned versions is
    recommended to eliminate dependency drift across replication attempts.
\end{itemize}

\subsection*{Hyperparameters}
\begin{itemize}[noitemsep]
  \item[\checkmark] \textbf{All hyperparameters reported.}
    Learning rate $\alpha=0.1$, discount $\gamma=0.99$, exploration
    schedule ($\varepsilon_0=1.0$, decay $0.9975$/episode, floor $0.01$),
    wear-off rate $\delta=0.1$, training length ($8{,}000$ or $20{,}000$
    episodes), and wait penalty $c_w \in \{-1, 0\}$ are all stated in
    Appendix~\ref{app:experimental}.
  \item[\checkmark] \textbf{No hidden hyperparameter search.}
    Hyperparameters were fixed before experiments; no tuning was performed
    across variants.
\end{itemize}

\subsection*{Experimental Rigour}
\begin{itemize}[noitemsep]
  \item[\checkmark] \textbf{Multiple independent seeds.}
    All results are averaged over $15$ seeds (variant comparisons) or
    $5$ seeds (critical-point sweep).
  \item[\checkmark] \textbf{Strategy classification is deterministic.}
    A run is classified as ``drinks'' based on the greedy ($\varepsilon=0$)
    rollout, not stochastic rollouts, removing rollout noise from the
    classification step.
  \item[$\circ$] \textbf{Statistical significance.}
    Confidence intervals or standard error bars are not currently shown
    on Table~\ref{tab:main_results}. Standard errors across the $15$
    seeds should be reported in the final version.
  \item[$\circ$] \textbf{Compute requirements stated.}
    Wall-clock training time and hardware (CPU/GPU model) are not reported.
    These should be added to Appendix~\ref{app:experimental} for full
    reproducibility.
\end{itemize}

\subsection*{Claims and Proofs}
\begin{itemize}[noitemsep]
  \item[\checkmark] \textbf{Analytical derivations verified.}
    The critical-point derivation (\S\ref{sec:critical}) is elementary
    algebra; all steps are shown in equations~(\ref{eq:v_sober}),
    (\ref{eq:v_drink}), and~(\ref{eq:wstar}).
  \item[\checkmark] \textbf{Analytical values match empirical trends.}
    Table~\ref{tab:critical} compares analytical $W^*$ to empirically
    observed majority-vote transitions; the partial-sobriety discrepancy
    is explained in \S\ref{sec:partial_sobriety}.
  \item[\checkmark] \textbf{Limitations stated.}
    Three limitations (scalability, stochastic wear-off, richer impairment
    models) are discussed in \S\ref{sec:discussion}.
  \item[$\circ$] \textbf{Proofs reviewed by a second author.}
    This is a single-author submission; independent verification of the
    analytical derivations is recommended prior to publication.
\end{itemize}

\subsection*{Data}
\begin{itemize}[noitemsep]
  \item[\checkmark] \textbf{No external datasets used.}
    All results come from interactions with the implemented gridworld;
    no proprietary or external dataset is required to replicate results.
  \item[\checkmark] \textbf{Raw sweep data provided.}
    Appendix~\ref{app:sweep} contains the full critical-point sweep table
    with Q-values and per-seed drink counts.
\end{itemize}
